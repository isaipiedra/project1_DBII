...
const MONGO_URI = 'mongodb://localhost:27017/miBaseDeDatos?directConnection=true';
...
//Función para obtener un dataset por el id
...
// Lista datasets de un autor
// Sin límite
async function getDatasetsByAuthor({ limit = 0, skip = 0 } = {}, autor) {
  return DataSet.find({ author: autor })
    .skip(skip)
    .limit(limit)
    .lean();
}

// Lista datasets con status "Aprobado"
// Sin límite
...
// Lista datasets con status "Pendiente"
// Sin límite
async function getPendingDatasets({ limit = 0, skip = 0 } = {}) {
  return DataSet.find({ status: 'Pendiente' })
    .skip(skip)
    .limit(limit)
    .lean();
}
...
async function insertDataSetGridFS({
  name, description, date, author, status,
...
export { connectMongo, getBucket, insertDataSetGridFS, DataSet, approveDataSet,
   deleteDataSet, getDatasetById, getDatasetsByName, cloneDatasetById, getApprovedDatasets, 
   getPendingDatasets, getDatasetsByAuthor};
...
(async () => {
  try {
    await connectMongo();
    const dsActualizado = await approveDataSet('68e1665b0ce6835d55d3f1c5');
    console.log('Aprobado:', dsActualizado._id.toString(), dsActualizado.status);
  } catch (e) {
...
(async () => {
  try {
    await connectMongo();
    const results = await getDatasetsByAuthor({}, 'Gabriel');
...
//68e16836fc151d27e963ca15
//68e1665b0ce6835d55d3f1c5
//68e169c55e98bbfe74f89e47
